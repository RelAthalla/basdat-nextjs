...
export default function DeleteHabitatButton({ habitat }: { habitat: Habitat }) {
  ...
  async function handleDelete() {
    setIsLoading(true);
    try {
      await deleteHabitat(habitat.id);
      router.refresh();
      toggleModal();
    } catch (error) {
      console.error(error);
  ...
  return (
    <div>
      <button className="btn btn-error btn-sm" onClick={toggleModal}>Delete</button>
      ...
      <div className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg">
            Are you sure you want to delete {habitat.namaHabitat}?
          </h3>
