...
export async function POST(request: Request) {
    try {
        ...
        const timestamp = new Date(jadwal).toISOString();

        await query(
            `INSERT INTO PAKAN (id_hewan, jadwal, jenis, jumlah, status)
             VALUES ($1, $2, $3, $4, $5)`,
            [idHewan, timestamp, jenisPakan, jumlahPakan, 'Menunggu Pemberian']
        );
...
export async function PUT(request: Request) {
    try {
        ...
        const result = await query(
            `UPDATE PAKAN
             SET jenis=$1, jumlah=$2, status=$3
             WHERE id_hewan=$4 AND jadwal=$5`,
            [jenisPakan, jumlahPakan, status || 'Menunggu Pemberian', idHewan, jadwal]
        );
